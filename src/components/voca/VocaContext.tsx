...
interface VocaContextType {
    ...
    // Chat
    sendMessage: (chatId: string, content: string, type?: 'text' | 'image' | 'voice' | 'video' | 'doc' | 'call' | 'poll' | 'event' | 'audio' | 'contact', mediaUrl?: string, duration?: string, replyToId?: string) => Promise<Message | undefined>;
...
export const VocaProvider = ({ children }: { children: ReactNode }) => {
    ...
    const sendMessage = useCallback(async (chatId: string, content: string, type: 'text' | 'image' | 'voice' | 'video' | 'doc' | 'call' | 'poll' | 'event' | 'audio' | 'contact' = 'text', mediaUrl?: string, duration?: string, replyToId?: string): Promise<Message | undefined> => {
                    ...
                    messages: chat.messages.map(m => {
                        if (m.id === messageId) {
                            ...
                            const newStarredBy = isStarred
                                ? m.starredBy?.filter(id => id !== currentUser.id)
